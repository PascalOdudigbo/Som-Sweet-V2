...
export async function createOrder(orderData: { paymentIntentId: string, shippingAddressId: number }): Promise<OrderType> {
  try {
    // Getting the user auth token from the local storage
...
// A function to get all the users orders
...
// A function to update the status of an order
...
// A function to delete all unpaid and pending orders
