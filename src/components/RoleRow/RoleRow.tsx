...
function RoleRow({ role, setRoles, editingRole, setEditingRole, handleEditRole }: RoleRowProps) {
  // State f=variable to control dropdown content display
  const [dropdownDisplay, setDropdownDisplay] = useState<string>("none")

  // A function to handle role deletion onClck event
  ...
  // Confirming if intent is editting or adding
  ...
  return (
    <tr className="row_wrapper">
      ...
      <td className="row_cell">
        {
          ((role.name.toLowerCase() === "administrator") || (role.name.toLowerCase() === "customer")) ?
            <></> :
